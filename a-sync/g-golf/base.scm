...
;; run in the default glib main loop thread and not in a thread pool
;; thread.  Exceptions thrown by the handler procedure will propagate
			...
			(lambda ()
			  (let ((res (thunk)))
			    (g-idle-add (lambda () (resume res) #f)
					'default)))
			(lambda args
			  (g-idle-add (lambda () (resume (apply handler args)) #f)
				      'default)))
      (thread-pool-add! pool
			(lambda ()
			  (let ((res (thunk)))
...
;; exception and 'handler' is run (the 'guile-a-sync-thread-error
;; symbol is reserved to the implementation and should not be yielded
;; by the generator).  Exceptions thrown by the handler procedure will
...
;; if not caught locally, will propagate out of g-main-loop-run.
;;
;; This procedure is first available in version 0.19 of this library.
